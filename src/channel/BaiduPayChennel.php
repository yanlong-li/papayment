...
class BaiduPayChennel extends Channel
{
    ...
    {
        $data = [
            'appKey' => $this->config['pay_app_key'],
            'dealId' => $this->config['pay_deal_id'],
            'tpOrderId' => $outTradeNo,
            'totalAmount' => strval($totalFee * 100),
        ];
        $data['rsaSign'] = self::sign($data, $this->config['private_key']);
    ...
    {
        $resultData = $data;
        return self::checkSign($data, $this->config['public_key']);
    }
    ...
    {
        if (!isset($assocArr['rsaSign']) || empty($rsaPubKeyStr)) {
            return false;
        }
